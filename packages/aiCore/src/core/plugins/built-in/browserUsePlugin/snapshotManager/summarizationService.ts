...
export class DefaultSummarizationService implements SnapshotSummarizationService {
  ...
  private generateBasicSummary(
    ...
    diff?: { isSignificant: boolean; urlChanged: boolean; contentChangeRatio: number }
  ): string {
    ...
    const hasLogin =
      snapshot.content.toLowerCase().includes('login') || snapshot.content.toLowerCase().includes('sign in')
